...
- A piece's head must never be its widest part and the neck must pinch, or the silhouette reads as a baby. The knight is the deliberate exception (its muzzle IS the widest point) and it only escapes the trap because its neck is half the head's width — 16 blocks to 8. A back edge that runs flat for many rows reads as a slab, not a creature; the curve and the mane notches are load-bearing, not decoration.
